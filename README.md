...
4. **Writes the code** (Main logic, Requirements, Documentation) to reproduce the paper's key results.
5. Employs a **Self-Refinement Loop** to review its own code and fix bugs.
6. Runs the code in an **Execution Sandbox** to verify functionality.
7. Packages the result into a downloadable `.zip` or pushes it directly to **GitHub**.
...
- **Agentic Workflow:** A 10-step pipeline utilizing LLMs for reasoning, ranking, writing, and self-correction.
...
- **Data Integration:** Semantic Scholar API, DuckDuckGo Search (Fallback), GitHub REST API
- **Infrastructure:** SQLite, Subprocess Sandboxing, SSE (Server-Sent Events)
